...
# # Etract Deck from Cabin number
# all_data$Deck<-sapply(all_data$Cabin, function(x) strsplit(x,NULL)[[1]][1])
# deck.fit<-rpart(Deck~Pclass+Fare,data=all_data[!is.na(all_data$Deck),])
...
# 
# # Extract Position from Cabin number
# all_data$CabinNum<-sapply(all_data$Cabin,function(x) strsplit(x,'[A-Z]')[[1]][2])
# all_data$num<-as.numeric(all_data$CabinNum)
# num<-all_data$num[!is.na(all_data$num)]
# Pos<-kmeans(num,3)
# all_data$CabinPos[!is.na(all_data$num)]<-Pos$cluster
# all_data$CabinPos<-factor(all_data$CabinPos)
# levels(all_data$CabinPos)<-c('Front','End','Middle')
# all_data$num<-NULL
